  ...
  public class Noise
  {
    public float[] Calc1D(int width, float scale)
    {
      ...
      for (var i = 0; i < width; i++)
        values[i] = (Generate(i * scale) * 128 + 128) / 256;
      return values;
    }
    ...
    public float[,] Calc2D(int width, int height, float scale)
    {
      ...
      for (var i = 0; i < width; i++)
        for (var j = 0; j < height; j++)
          values[i, j] = (Generate(i * scale, j * scale) * 128 + 128) / 256; //division makes this from 0 - 1 instead of from 0 - 255
      return values;
    }
      ...
      for (var i = 0; i < width; i++)
        for (var j = 0; j < height; j++)
          for (var k = 0; k < length; k++)
            values[i, j, k] = (Generate(i * scale, j * scale, k * scale) * 128 + 128) / 256;
      return values;
    }
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage("Major Code Smell", "S3358:Ternary operators should not be nested", Justification = "Who fucking cares, really, its single use")]
    ...
    /// <summary>
    /// apparently not in use, might remove
    /// </summary>
